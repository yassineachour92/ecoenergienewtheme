		...
		<table class="features-table">
			...
			for ( $i = 1; $i <= $attributes_number; $i++ ) {
				...
				if ( !empty( $attribute_value ) ) {
					...
					?>
					<tr>
						<td class="attribute-label-single"><?php echo $label ?></td>
						<td class="attribute-value-unit-single"><span class="attribute-value-single"><?php echo apply_filters( 'ic_catalog_attr_val_display', $attribute_value, $label, $i, $product_id ) ?></span> <span class="attribute-unit-single"><?php echo get_attribute_unit( $i, $product_id ) ?></span></td>
					</tr>
					<?php
				}
			}
