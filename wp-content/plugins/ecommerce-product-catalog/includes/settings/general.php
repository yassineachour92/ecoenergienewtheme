					...
					if ( !is_advanced_mode_forced() || ic_is_woo_template_available() || is_ic_shortcode_integration() ) {
						...
						<div class="ic-important-settings">
							...
							if ( ic_has_listing_shortcode() ) {
								implecode_info( sprintf( __( 'You are currently using %s on your product listing to integrate the catalog with the theme.', 'ecommerce-product-catalog' ), ic_catalog_shortcode_name() ) . '</p>' . '<p>' . __( 'If you have any problems with catalog layout you can remove the shortcode and use the theme integration wizard to fix it.', 'ecommerce-product-catalog' ) . '</p>' );
...
function ic_shortcode_mode_settings() {
	...
	if ( is_ic_shortcode_integration() ) {
		...
		implecode_settings_checkbox( __( 'Show main catalog page content everywhere', 'ecommerce-product-catalog' ), 'archive_multiple_settings[shortcode_mode][show_everywhere]', $settings[ 'show_everywhere' ], 1, __( 'Check this if you want to display main catalog page content on every catalog page. For example if you are using page builder on main catalog page to design your catalog.', 'ecommerce-product-catalog' ) );
		...
		if ( is_ic_breadcrumbs_enabled() ) {
			implecode_settings_checkbox( __( 'Move breadcrumbs top the top', 'ecommerce-product-catalog' ), 'archive_multiple_settings[shortcode_mode][move_breadcrumbs]', $settings[ 'move_breadcrumbs' ], 1, __( 'Breadcrumbs will be displayed before page title. It may require some additional styling when checked.', 'ecommerce-product-catalog' ) );
		}
